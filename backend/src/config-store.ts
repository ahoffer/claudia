...
const DEFAULT_MCP_SERVERS: MCPServerConfig[] = [
    {
        name: 'playwright',
        command: 'npx',
        args: ['@playwright/mcp', '--browser', 'chromium', '--ignore-https-errors'],
        enabled: true
    }
];
...
export class ConfigStore {
    ...
    // Upgrade saved MCP server entries to match current defaults where needed.
    // Upgrade saved playwright MCP entries to match current required flags.
    private migrateMcpServers(servers: MCPServerConfig[]): MCPServerConfig[] {
        return servers.map(s => {
            if (s.name === 'playwright' && s.command === 'npx') {
                let args = [...(s.args ?? [])];
                if (!args.includes('--browser')) {
                    args = [...args, '--browser', 'chromium'];
                }
    ...
    private loadConfig(): AppConfig {
        try {
            if (existsSync(this.configFile)) {
                ...
                const mcpServers = this.migrateMcpServers(
