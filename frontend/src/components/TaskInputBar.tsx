...
export function TaskInputBar({ task, wsRef }: TaskInputBarProps) {
    ...
    const handleBlur = () => {
        // Use setTimeout to allow click events to fire first (so focus on another
        // input overwrites this before we clear it).
        // When voice mode is active, keep focusedInputId sticky so clicking the
        // Voice button or anywhere else doesn't break transcript routing.
        setTimeout(() => {
            if (useTaskStore.getState().globalVoiceEnabled) return;
